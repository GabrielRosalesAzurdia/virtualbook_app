...
class _LibrosDetailsState extends State<LibrosDetails> {
  ...
  Future<String> getAutor() async {
    await getToken().then((value) async {
      var token = value;
      var res = await http.get(Uri.encodeFull("https://virtualbook-backend.herokuapp.com/api/autores/" + autorIDd.toString() + "/"),headers: {"Authorization":"JWT "+token.token} );
      try{
        setState(() {
  ...
  Future<String> getCategory() async {
    await getToken().then((value) async {
      var token = value;
      var res = await http.get(Uri.encodeFull("https://virtualbook-backend.herokuapp.com/api/categorys/" + categoriaId.toString() + "/"),headers: {"Authorization":"JWT "+token.token} );
      try{
        setState(() {
          var resBody = json.decode(res.body);
  ...
  Future<String> addToDeseos() async {
    ...
    await getToken().then((value) async {
      ...
      var res = await http.post(
        Uri.encodeFull("https://virtualbook-backend.herokuapp.com/api/CrearLista/deseos"),
  ...
  Future<String> addToRentados() async {
    ...
    await getToken().then((value) async {
      var token = value;
      var res = await http.post(
        Uri.encodeFull("https://virtualbook-backend.herokuapp.com/api/CrearLista/renta"),
        headers: {"Authorization":"JWT "+token.token},
        body: {
          "cart_id":usuarioId.toString(),
          "book_id":id.toString(),
