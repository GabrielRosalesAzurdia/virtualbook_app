...
class _ListaDeseosState extends State<ListaDeseos> {
  ...
  Future<String> deleteElementDeseos(idLista) async {
    await getToken().then((value) async {
      var token = value;
      var res = await http.delete(Uri.encodeFull("https://virtualbook-backend.herokuapp.com/api/deleteLista/one/deseos/"+  idLista.toString() + "/"),headers: {"Authorization":"JWT "+token.token} );
      var resBody = json.decode(res.body);
