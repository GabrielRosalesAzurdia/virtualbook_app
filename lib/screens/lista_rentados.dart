...
class _ListaRentadosState extends State<ListaRentados> {
  ...
  Future<String> revicionFechas() async {
    await getToken().then((value) async {
      var token = value;
      var res = await http.get(Uri.encodeFull("https://virtualbook-backend.herokuapp.com/api/rentas/checkDates/"+  usuarioId.toString() + "/"),headers: {"Authorization":"JWT "+token.token} );
      var resBody = json.decode(res.body);
  ...
  Future<String> deleteElementRentados(idLista) async {
    await getToken().then((value) async {
      var token = value;
      var res = await http.delete(Uri.encodeFull("https://virtualbook-backend.herokuapp.com/api/deleteLista/one/renta/"+  idLista.toString() + "/"),headers: {"Authorization":"JWT "+token.token} );
      var resBody = json.decode(res.body);
      if(resBody == "Se ha eliminado"){
